...
if __name__ == "__main__":
    ...
    url = f"https://airhdl.com/api/registermaps/{register_map_id}?includeRegisters=false"
    req = urllib.request.Request(
        url=url, method="GET")
    ...
    url = f'https://airhdl.com/api/registermaps/{register_map_id}/download?type={file_type}'
    req = urllib.request.Request(
        url=url, method="GET")
    req.add_header("authorization", f"Bearer {token}")
    resp = urllib.request.urlopen(req, timeout=5)
